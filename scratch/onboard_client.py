...
parser = argparse.ArgumentParser(description="Onboard new tenants and push their Zoho CRM credentials to Vercel KV (Encrypted)")
...
parser.add_argument("--phone_id", help="Meta Phone Number ID mapped to this tenant")
...
print("Please enter the client Zoho credentials interactively. Inputs for secret keys will be hidden.")
...
if not client_id or not client_secret or not refresh_token:
    print("[ONBOARDER] Error: Credentials cannot be empty.")
    exit(1)

# Encrypt sensitive keys before writing to database
enc_client_id = encrypt_value(client_id)
enc_client_secret = encrypt_value(client_secret)
enc_refresh_token = encrypt_value(refresh_token)

# Construct CRM payload object
creds = {
...
# 2. Save optional Phone ID mapping
if args.phone_id:
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            parsed = json.loads(res_data)
            if parsed.get("result") == "OK":
                print(f"[ONBOARDER] Mapped Phone ID: {args.phone_id.strip()} -> {args.id}")
    except Exception as e:
        print(f"[ONBOARDER] Exception saving Phone ID mapping: {e}")

# 3. Save optional Hostname mapping
if args.hostname:
